...
class TestGetFeedbacks:
    ...
    async def test_get_feedbacks_empty(self, client: AsyncClient):
        """无工单时返回空列表"""
        resp = await client.get("/api/feedback")
        assert resp.status_code == 200
        assert resp.json() == []

    async def test_get_feedbacks_with_data(self, client: AsyncClient, db_session: AsyncSession):
        """有工单时返回数据"""
        user, fb = await seed_feedback(db_session)
        resp = await client.get("/api/feedback", params={"user_id": user.id})
        assert resp.status_code == 200
        data = resp.json()
        assert len(data) == 1
        assert data[0]["title"] == "测试工单"

    async def test_get_feedbacks_filter_by_status(self, client: AsyncClient, db_session: AsyncSession):
        """按状态筛选"""
        user, fb = await seed_feedback(db_session)
        resp = await client.get("/api/feedback", params={"status": "pending"})
        assert resp.status_code == 200
        data = resp.json()
        assert len(data) == 1

        resp = await client.get("/api/feedback", params={"status": "resolved"})
        assert resp.status_code == 200
...
class TestCreateFeedback:
    ...
    async def test_create_feedback_basic(self, client: AsyncClient, db_session: AsyncSession):
        """创建基本工单"""
    ...
    async def test_create_feedback_title_required(self, client: AsyncClient):
        """标题必填 - FormData 验证"""
        resp = await client.post("/api/feedback", data={
            "content": "没有标题",
        })
