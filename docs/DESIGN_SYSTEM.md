...
学员端不是教学系统，也不是运营数据看板。用户购买后只需要选择工具、点击一次、等待自动处理。

- 登录后默认进入工具箱，不经过数据总览。
- 工具卡片整卡可点，一个工具只保留一个主操作。
- 不展示成功率、使用趋势、工具分布、内部步骤、技术错误栈和预计耗时。
- 运行中只展示“准备环境、自动处理、检查结果、完成”四段业务进度。
- 只有脚本确实无法代办时才请求用户完成一步，并提供唯一的“我已完成，继续处理”按钮。
- 套餐未包含使用亚马逊橙引导升级；维护、失败、停止必须使用不同状态文案。
- 普通活动和版本公告使用页内横幅，只有系统与维护类重要公告允许阻断。
- 成功率等质量指标属于管理员内部诊断信息，不进入学员端界面或套餐营销文案。
