    ...
    >
      <template #actions>
        <div class="hero-status">
          <span :class="{ online: isDesktop }"></span>
          <div><strong>{{ isDesktop ? '本地解析器已连接' : '需要桌面客户端' }}</strong><small>Excel 原文件不上传，后台只保存标准化规则</small></div>
        </div>
      </template>
    </PageHeader>
      ...
      <div class="work-column">
        <article class="panel upload-panel">
          ...
          <div class="upload-actions">
            <button class="primary" :disabled="busy || !isDesktop" @click="chooseWorkbook"><Upload :size="17" />选择新费率表</button>
            <button :disabled="busy || !isDesktop" @click="loadBuiltIn"><Database :size="17" />载入内置首版</button>
          </div>
...
import type { FreightQuoteResult, FreightRatePack } from '@/shared/freight/types'
import PageHeader from '@/components/PageHeader.vue'

interface Mapping { carrierKey: string; carrierName: string; worksheetName: string | null; confidence: number; ruleCount: number }
interface ParsedWorkbook { pack: FreightRatePack; sourceFileName: string; availableWorksheets: string[]; mappings: Mapping[]; warnings: string[]; summary: { carrierCount: number; countryCount: number; ruleCount: number } }
...
const isDesktop = computed(() => Boolean(window.electronAPI?.freight))
...
function parsePayload(value: unknown): ParsedWorkbook {
...
async function chooseWorkbook() {
  busy.value = true
  try { const value = await window.electronAPI?.freight?.parseWorkbook(options()); if (value) acceptParsed(value) }
...
async function loadBuiltIn() {
  busy.value = true
  try { acceptParsed(await window.electronAPI?.freight?.getDefaultPack()) }
...
async function reparse() {
  busy.value = true
  try { acceptParsed(await window.electronAPI?.freight?.reparseWorkbook(options())) }
...
async function calculateQuote() {
  ...
  busy.value = true
  try {
    quoteResult.value = await window.electronAPI?.freight?.quote({
