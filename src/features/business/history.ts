import { z } from 'zod'
import { serverBatchHistorySchema } from './model'
...
export interface HistoryItem { label: string; status: string; result: string }
...
export function parseHistoryDetail(kind: 'demo' | 'live', value: unknown): HistoryDetail {
  if (kind === 'demo') {
    ...
    return { kind, toolName: batch.tool_name_snapshot, toolId: String(batch.tool_id), status: batch.status, total: batch.row_count,
      items: batch.items.map((item, index) => ({ label: `演示项 ${index + 1}`, status: demoStatus[item.status] || '状态待确认',
  ...
  return { kind, toolName: batch.tool_name, toolId: batch.tool_id === undefined ? undefined : String(batch.tool_id), status: batch.status, total: batch.total_count,
    ...
    items: batch.items.map((item, index) => ({ label: `账号 ${index + 1}`, status: liveStatus[item.status] || '状态待确认',
...
export function historyCsv(detail: HistoryDetail): string {
  ...
  const rows = [['类型', '账号序号', '状态', '结果说明'], ...detail.items.map(item => [detail.kind === 'demo' ? '模拟演示' : '真实执行', item.label, item.status, item.result])]
