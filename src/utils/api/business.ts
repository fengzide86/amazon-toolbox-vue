...
type ActionCenterData = Schemas['ActionCenterData']
...
export const createBusinessBatch = (payload: Schemas['BatchCreate']): Promise<BusinessBatchEnvelope> => api.post('/api/business/batches', payload)
export const updateBusinessBatch = (batchId: EntityId, payload: Schemas['BatchUpdate']): Promise<BusinessBatchEnvelope> => api.patch(`/api/business/batches/${batchId}`, payload)
export const updateBusinessBatchItem = (batchId: EntityId, itemId: string, payload: Schemas['BatchItemUpdate']): Promise<BusinessBatchItemEnvelope> =>
  api.put(`/api/business/batches/${batchId}/items/${encodeURIComponent(itemId)}`, payload)
export const finishBusinessBatch = (batchId: EntityId, status: Schemas['BatchFinish']['status']): Promise<BusinessBatchEnvelope> =>
  api.post(`/api/business/batches/${batchId}/finish`, { status })
export const getAdminActionCenter = (): Promise<ActionCenterData> => api.get('/api/admin/action-center', {}, { cache: false })
export const getAdminBusinessBatch = (batchId: EntityId): Promise<never> => api.get(`/api/admin/business-batches/${batchId}`, {}, { cache: false })
